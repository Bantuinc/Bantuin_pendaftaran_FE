...
const getMemberInformation = async (userId: string, accessToken: string,teamMemberId:string) => {
    try {
...
async function TeamDetail({ params }: { params: { teamId: string,teamMemberId:string } }) {
    const accessToken = cookies().get("accessToken")?.value as string;
    const member = await getMemberInformation(params.teamId, accessToken,params.teamMemberId);
        ...
        <div className="lg:flex">
            ...
            <div className={`${hind.className} flex-1 px-2 sm:px-0 py-3 md:py-0`}>
                ...
                <div className="flex md:flex-row flex-col gap-6">
                    <MemberForm teamId={params.teamId} member={member} additionalField={additionalField} isAdd={false}/>
                    <DeleteButtonForm competitionId={params.teamId} teamId={params.teamMemberId} accessToken={accessToken}/>
                </div>
            </div>
        </div>
    );
